...
public class Lab2 {
    ...
    public static void testAlg(ThreadLogic threadLogic) throws FileNotFoundException {
        ...
        for (int i = 0; i < MATRIX_COUNT; i++) {
            matrixTestsA[i] = MatrixFunks.readMatrixFile("src/main/resources/test10/a_" + i + ".txt", SIZE);
            matrixTestsB[i] = MatrixFunks.readMatrixFile("src/main/resources/test10/b_" + i + ".txt", SIZE);
            matrixTestsResults[i] = MatrixFunks.readMatrixFile("src/main/resources/test10/c_" + i + ".txt", SIZE);
        }
